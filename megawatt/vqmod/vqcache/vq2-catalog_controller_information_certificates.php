...
class ControllerInformationCertificates extends Controller {
   private $error = array();
     ...
     public function index() {
      $this->language->load('information/certificates'); //Optional. This calls for your language file
      ...
      if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/certificates.tpl')) { //if file exists in your current template folder
         $this->template = $this->config->get('config_template') . '/template/information/certificates.tpl'; //get it
      } else {
         $this->template = 'default/template/information/certificates.tpl'; //or get the file from the default folder
      }
