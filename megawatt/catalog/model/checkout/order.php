...
class ModelCheckoutOrder extends Model {
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			$template->data['link'] = $order_info['store_url'] . 'my-account/';
//update
			$template->data['text_greeting_part_1'] = $language->get('text_greeting_part_1');
			$template->data['text_greeting_part_2'] = $language->get('text_greeting_part_2');
			$template->data['text_greeting_part_3'] = $language->get('text_greeting_part_3');
            $template->data['text_contact_title'] = $language->get('text_contact_title');
			$template->data['phone_contact_1'] = current($this->config->get('marketshop_top_bar_contact'));
			$template->data['phone_contact_2'] = current($this->config->get('marketshop_top_bar2_contact'));
			$template->data['phone_contact_3'] = current($this->config->get('marketshop_top_bar3_contact'));
			$template->data['mail_contact'] = current($this->config->get('marketshop_top_bar_email'));
//end
			...
			if ($this->config->get('config_alert_mail')) {
				...
				$text .= $language->get('text_new_customer_info') . '<br>' . $language->get('text_new_firstname') . $order_info['payment_firstname'] . '<br>';
				$text .= $language->get('text_new_lastname') . $order_info['payment_lastname'] . '<br>';
				$text .= $language->get('text_new_telephone') . $order_info['telephone'] . '<br><br>';

				$text .= $language->get('payment_method') . $order_info['payment_method'] . '<br>';
				$text .= $language->get('shipping_method') . $order_info['shipping_method'] . '<br>';
				if($order_info['shipping_address_1']){
				$text .= $language->get('shipping_address_1') . $order_info['shipping_address_1'] . '<br>';
				};
				if($order_info['shipping_city']){
				$text .= $language->get('shipping_city') . $order_info['shipping_city'] . '<br>';
				};
				if($order_info['shipping_zone']){
				$text .= $language->get('shipping_zone') . $order_info['shipping_zone'] . '<br>';
				};
				if($order_info['shipping_country']){
				$text .= $language->get('shipping_country') . $order_info['shipping_country'] . '<br><br>';
				};
				...
				$text .= '<b>' . $language->get('text_new_products') . '</b><br>';
