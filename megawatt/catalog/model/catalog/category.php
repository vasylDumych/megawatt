...
class ModelCatalogCategory extends Model {
    ...
    public function getMinPriceFromCategory($category_id) {
        $sql = 'SELECT LEAST(p.price,IFNULL(ps.price, p.price)) min_price
                ...
                LEFT JOIN '.DB_PREFIX.'product p ON '.DB_PREFIX.'p.product_id = ' . DB_PREFIX . 'p2c.product_id  
                LEFT JOIN '.DB_PREFIX.'product_special ps ON p.product_id = ps.product_id               
                WHERE ' . DB_PREFIX .'cp.path_id = '.(int)$category_id.' AND p.status = 1 AND p.price > 0
                ORDER BY min_price
                LIMIT 0,1';
