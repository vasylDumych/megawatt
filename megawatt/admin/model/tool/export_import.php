...
class ModelToolExportImport extends Model {
	...
	protected function getSeries() {
		// find all series already stored in the database
		$series_ids = array();
		$sql  = "SELECT ss.series_id, ss.store_id, s.`name` FROM `".DB_PREFIX."series_to_store` ss ";
		$sql .= "INNER JOIN `".DB_PREFIX."series` s ON s.series_id=ss.series_id";
		$result = $this->db->query( $sql );
		$series = array();
		foreach ($result->rows as $row) {
			...
			if (!isset($series[$name]['series_id'])) {
				$series[$name]['series_id'] = $series_id;
			}
			if (!isset($series[$name]['store_ids'])) {
				$series[$name]['store_ids'] = array();
			}
			if (!in_array($store_id,$series[$name]['store_ids'])) {
				$series[$name]['store_ids'][] = $store_id;
			}
		}
		return $series;
	}
	...
	protected function storeSeriesIntoDatabase( &$series, $name, $manufacturer_id, &$store_ids, &$available_store_ids ) {
		foreach ($store_ids as $store_id) {
			if (!in_array( $store_id, $available_store_ids )) {
				continue;
			}
			if (!isset($series[$name]['series_id'])) {
				$this->db->query("INSERT INTO ".DB_PREFIX."series SET name = '".$this->db->escape($name)."', manufacturer_id = '" . (int)$manufacturer_id . "', image='', sort_order = '0'");
			...
			if (!in_array($store_id,$series[$name]['store_ids'])) {
				...
				$sql = "INSERT INTO `".DB_PREFIX."series_to_store` SET series_id='".(int)$series_id."', store_id='".(int)$store_id."'";
				$this->db->query( $sql );
				$series[$name]['store_ids'][] = $store_id;
			}
		}
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$series, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		if ($series_name) {
			$this->storeSeriesIntoDatabase( $series, $series_name, $manufacturer_id, $store_ids, $available_store_ids );
			$series_id = $series[$series_name]['series_id'];
		} else {
			$series_id = 0;
		}
		...
		if (count($series_id)) {
			$sql = "INSERT INTO `".DB_PREFIX."product_to_series` (`product_id`,`series_id`) VALUES ($product_id,$series_id)";
			$this->db->query($sql);
		}
	...
	protected function deleteProducts( $exist_table_product_tag, &$url_alias_ids ) {
		...
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."product_to_series`;\n";
	...
	protected function deleteProduct( $product_id, $exist_table_product_tag ) {
		...
		$sql .= "DELETE FROM `".DB_PREFIX."product_to_series` WHERE `product_id` = '$product_id';\n";
	...
	protected function getProductOldPrice($productId) {
        $query = $this->db->query("SELECT product_id, price FROM `".DB_PREFIX."product` WHERE product_id='".(int)$productId."'");

        return $query->row['price'];
    }

    // get the product special
	protected function getProductSpecial($productId) {
        $sql  = "SELECT ps.*";
        $sql .= "FROM `".DB_PREFIX."product_special` ps ";
        $sql .= "WHERE ps.product_id ='".(int)$productId."' LIMIT 1";
        $result = $this->db->query($sql);
	...
	protected function deleteProductSpecials($productId) {
        $sql = "DELETE FROM `".DB_PREFIX."product_special` WHERE product_id='".(int)$productId."'";
        $this->db->query($sql);
    }

    // store product specials
	protected function storeProductSpecial($productId, $price) {
        $priority = '0';
        ...
        $sql .= "($productId,$customerGroupId,$priority,$price,'$dateStart','$dateEnd')";
        $res = $this->db->query($sql);
    }

	// store product special (if needed)
	protected function moreProductSpecial( &$product, $oldPrice ) {
        $productId = $product['product_id'];
        $oldProductSpecial = $this->getProductSpecial($productId);

        if (isset($oldProductSpecial['price'])) {

            if ($oldProductSpecial['price'] != $product['price']) {
                //delete product specials
                $this->deleteProductSpecials($productId);
            }

            // check product special and new price
            if ($oldProductSpecial['price'] > $product['price']) {
                //store new product specials
                $this->storeProductSpecial($productId, $product['price']);
                $product['price'] = $oldProductSpecial['price'];
            }
		} else {
            // check product price and new price
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		// find existing series, only newly specified series will be added
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$series_name = $this->getCell($data,$i,$j++);
			...
			$oldPrice = $this->getProductOldPrice($product_id);
			if ($incremental) {
				$this->deleteProduct( $product_id, $exist_table_product_tag );
			}
			$available_product_ids[$product_id] = $product_id;
			$this->moreProductCells( $i, $j, $data, $product );
			$this->moreProductSpecial( $product, $oldPrice );
			$this->storeProductIntoDatabase( $product, $languages, $product_fields, $exist_table_product_tag, $exist_meta_title, $layout_ids, $available_store_ids, $manufacturers, $series, $weight_class_ids, $length_class_ids, $url_alias_ids );
		}
	}
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array_merge( $expected_heading, array( "location", "quantity", "model", "manufacturer", "series", "image_name", "shipping", "price", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "seo_keyword", "description") );
		if ($exist_meta_title) {
...
//		}
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		...
		$sql .= "LEFT JOIN `".DB_PREFIX."product_to_series` p2ss ON p2ss.product_id = p.product_id ";
		$sql .= "LEFT JOIN `".DB_PREFIX."series` s ON s.series_id= p2ss.series_id ";
	...
	function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('series'),10)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('image_name'),12)+1);
