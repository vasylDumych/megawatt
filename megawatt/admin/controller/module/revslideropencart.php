...
// end language
		...
		$this->data['action'] = $this->url->link('module/revslideropencart', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['revslideropencart_module'])) {
			$this->data['modules'] = $this->request->post['revslideropencart_module'];
		} elseif ($this->config->get('revslideropencart_module')) { 
			$this->data['modules'] = $this->config->get('revslideropencart_module');
		}	
...
ob_start();
	...
	$this->data['rev_content'] = ob_get_contents();
ob_end_clean();
//end revsslider custom code
