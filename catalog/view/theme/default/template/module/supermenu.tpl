...
$(document).ready(function(){
  ...
  $("#supermenu .tlli").hover(function() {
    $(".content-shadow").show().css("height", $(document).height());
  }, function() {
    $(".content-shadow").hide();
  });

//  $("#supermenu .tlli, #supermenu .bigdiv, #supermenu .inflyouttoright").hover(function() {
//    $("body").addClass("body-content-shadow");
//  }, function() {
//    $("body").removeClass("body-content-shadow")
//  });
...
<?php //if ($_SERVER['QUERY_STRING'] == '' || $_SERVER['QUERY_STRING'] == '_route_=ru/' ) { ?>
	<style>
		#supermenu {
		...
		#supermenu ul li.tlli.manufacturers {
			width: 145px;
		}

		#supermenu ul li.tlli.information-calc {
			width: 215px;
		}

		#supermenu ul li.tlli.information-news {
			width: 150px;
		}

		#supermenu ul li.tlli.specialprice {
			background: red;
			width: 200px;
		}

		#supermenu ul li.tlli.specialprice:hover a.tll {
			background: red;
			border-radius: 0;
		}

		#supermenu ul li.firstli{
			width: 100px;
		}

		#supermenu ul li.firstli:first-child {
			width: 100px;
		}
	</style>
<?php //} ?>
...
<div id="supermenu">
	<ul>
		...
		<?php foreach ($mitems as $mitem) { ?>
			<li class="firstli tlli<?php if ($mitem['children']) { ?> mkids<?php } ?><?php if ($mitem['cssid'] == 'login_drop') { ?> mkids lad<?php } ?><?php if ($mitem['tlstyle']) { ?> <?php echo $mitem['tlstyle']; ?><?php } ?>">
				<a class="tll" <?php if ($mitem['tlcolor']) { ?>style="color: <?php echo $mitem['tlcolor']; ?>;" <?php } ?><?php if (($mitem['children'] || ($mitem['chtml'] && $mitem['chtml'] == 1)) && $linkoftopitem != 'topitem') { ?><?php } else { ?> <?php if ($mitem['href']) { ?>href="<?php echo $mitem['href']; ?>"<?php } ?> <?php } ?>><?php echo $mitem['name']; ?></a>
...
-->
								<ul>
									<?php foreach ($mitem['fbrands'] as $dropbrand) { ?>
										<li class="supermenu-left"><a href="<?php echo $dropbrand['href']; ?>"><?php echo $dropbrand['name']; ?></a></li>
									<?php } ?>
								</ul>
						<?php } ?>
