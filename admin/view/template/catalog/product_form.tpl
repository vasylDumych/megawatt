...
$('input[name=\'manufacturer\']').autocomplete({
    ...
    $.ajax({
      url: 'index.php?route=catalog/product/getSeries&token=<?php echo $token; ?>&manufacturer_id=' +  encodeURIComponent(ui.item.value),
      dataType: 'json',
      success: function(json) {
        var html = '<option value="0" selected="selected"><?php echo $text_none; ?></option>';
        if (json['series'].length) {
          $.each(json.series, function() {
            html += '<option value="' + this.series_id + '">' + this.name + '</option>';
          });    
        }    
