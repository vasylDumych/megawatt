...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_series pts ON (p.product_id = pts.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				$sql .= " OR p.product_id LIKE '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
			}
    ...
    public function getRandomProductByCategoryId($product_id) {
        $limitDefault = 5;

        $productCategories = $this->getCategories($product_id);
        $firstCategory = $productCategories[0]['category_id'];

        $productData = array();

        $data = array(
            'filter_category_id' => $firstCategory
        );
        $results = array_values($this->getProducts($data));
        $countProductsInCategory = count($results);
        if ($countProductsInCategory <= 0) {
            return $productData;
        }

        $limit = $countProductsInCategory > $limitDefault ? $limitDefault : $countProductsInCategory;
        $idArr = array();

        while (count($idArr) < $limit) {
            $randomIndex = rand(0, $countProductsInCategory-1);
            if (!in_array($randomIndex, $idArr)) {
                $idArr[] = $randomIndex;
                $productData[] = $results[$randomIndex];
            }
        }
