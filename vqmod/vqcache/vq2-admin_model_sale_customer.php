...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', salt = '" . ((defined('GKD_UNIV_IMPORT') && !empty($data['salt'])) ? $this->db->escape($salt = $data['salt']) : $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9))) . "', password = '" . ((defined('GKD_UNIV_IMPORT') && !empty($data['salt'])) ? $this->db->escape($data['password']) : $this->db->escape(sha1($salt . sha1($salt . sha1($data['password']))))) . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
