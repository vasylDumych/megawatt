...
class ControllerModuleProductTab extends Controller {
	...
	public function index() {   
		...
		$this->data['action'] = $this->url->link('module/product_tab', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['modules'] = array();
		
		if (isset($this->request->post['product_tab_module'])) {
			$this->data['modules'] = $this->request->post['product_tab_module'];
		} elseif ($this->config->get('product_tab_module')) { 
			$this->data['modules'] = $this->config->get('product_tab_module');
		}				
 ...
 $this->data['layouts'][] = array('layout_id'=>0, 'name' => 'all' ); 
		...
		$this->template = 'module/product_tab.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
